...
if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) AND strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
    ...
    $MustHaveZone = ['mobile legends','mobile legend', 'lifeafter credits', 'ragnarok m: eternal love', 'tom and jerry : chase', 'knights of valour', 'scroll of onmyoji'];
    $produk = $row['name'];
    $keterangan = $row['note'];
   ...
   $config = $call->query("SELECT * FROM conf WHERE code='trxadmin'")->fetch_assoc();
    ...
    $price = ($data_user['level'] == 'Basic') ? $row['basic'] : $row['premium'];
...
$id = explode(' | ', filter($get_data))[0];
$server = explode(' | ', filter($get_data))['1'];
        $game = $row['brand'];
    ...
    if($game == "DANA"){
        ...
        $games = "callofduty";
    } else if ($game == "POINT BLANK"){
        $games = "pointblank";
    } else if ($game == "ARENA OF VALOR"){
        $games = "aov";
    } else if ($game == "HAGO"){
        $games = "hago";
    } else if ($game == "MOBILE LEGENDS"){
        $games = "mobilelegends";
    } else if ($game == "RAGNAROK M: ETERNAL LOVE"){
        $games = "ragnarok";
    }
    
    $ewallet_list = ['dana','ovo','grab','linkaja','shopeepay','gopay'];
    
    if (in_array($games, $ewallet_list)) {
        $data = $PaydayConfig->ewallet(strtoupper($games), $id);
        $name = $data->data->Name ?? '';
        // $url = "https://api-rekening.my.id/trueid/ewallet/$games/?hp=$id&key=83c8aae66f3c8ea";
    } else if (in_array($games, ['mobilelegends', 'ragnarok'])) {
        $data = $PaydayConfig->game($games, $server, $id);
        $name = $data->nickname ?? '';
        // $url = "https://api-rekening.my.id/trueid/game/mobilelegends/?id=$id&server=$server&key=83c8aae66f3c8ea";
    } else {
        $data = $PaydayConfig->game($games, null, $id);
        $name = $data->nickname ?? '';
        // $url = "https://api-rekening.my.id/trueid/game/$games/?id=$id&zone=$server&key=83c8aae66f3c8ea";
    }
    //print json_encode($data->data->Name);
    // $ch = curl_init();
    ...
    <div class="section s-invoice">
        <ul class="card listview rgs-listview image-listview">
            ...
            <ul class="listview rgs-listview-custom image-listview">
                ...
                <? endif ?>
<?php if (in_array($game, ['OVO', 'DANA', 'GO PAY', 'SHOPEE PAY', 'LINKAJA'])) : ?>
    <li>
        <div class="item">
            <div class="in">
                <div class="text-muted">Atas Nama</div>
                <?php if (!empty($name)) : ?>
                    <div class="rgs-text-rincian col-8"><?= $name ?></div>
                <?php else : ?>
                    <div class="rgs-text-rincian col-8">Harap Periksa Nomor Anda</div>
                <?php endif; ?>
            </div>
        </div>
    </li>
<?php endif; ?>
<?php if (in_array($games, ['freefire','callofduty','pointblank','aov','hago','mobilelegends','ragnarok'])) : ?>
    <li>
        <div class="item">
            <div class="in">
                <div class="text-muted">Nick Game</div>
                <?php if (!empty($name)) : ?>
                    <div class="rgs-text-rincian col-8"><?= $name ?></div>
                <?php else : ?>
                    <div class="rgs-text-rincian col-8">Harap Periksa ID Game Anda</div>
                <?php endif; ?>
            </div>
        </div>
    </li>
                ...
                <li>
                    <div class="item">
                        <div class="in">
                            ...
                            <div class="rgs-text-rincian"><?= 'Rp '.currency($biayaAdmin) ?></div>
                        </div>
                    </div>
                </li>
                <li>
                    <div class="item">
                        <div class="in">
                            ...
                            <div class="rgs-text-rincian"><b><?= 'Rp '.currency($price + $biayaAdmin) ?></b></div>
                        </div>
                    </div>
                </li>
            </ul>
                    ...
                    <input type="hidden" id="csrf_token" name="csrf_token" value="<?= $csrf_string ?>">
                        <div class="form-bottom-fixed">
                            ...
                            <div class="d-flex justify-content-between">
                                ...
                                <button type="submit" class="btn btn-primary btn-block btn-lg m-1 mb-0" name="<?= $actionPOST?>" onclick="var e=this;setTimeout(function(){e.disabled=true;},0);return true;">Konfirmasi</button>
                            </div>
                        </div>
                    </form>
                </div>
