    ...
    if($result_csrf == false) {
        ...
        if($call->query("SELECT * FROM users WHERE phone = '$postPhone'")->num_rows == 0) {
            ...
            if($data_user['status'] == 'suspend') {
                ...
                $result_wa = $WATL->sendMessage($postPhone, "*$PIN* adalah PIN Baru {$_CONFIG['title']} Anda.

Harap jangan pernah membagikan PIN Anda kepada siapa pun!");
                $result_mail = mailer($_MAILER,[
                    'dest' => $data_user['email'],
                    'name' => $data_user['name'],
                    'subject' => 'PIN Baru '.$_CONFIG['title'].'',
                    'message' => base64_encode(mailplate('pin',[
                        'name' => $data_user['name'],
                        'pin' => $PIN
                    ])),
                    'is_template' => 'yes'
...
if(isset($_POST['loginWhatsApp'])) {
    ...
    if($result_csrf == false) {
        ...
        if($call->query("SELECT * FROM users WHERE phone = '$postPhone'")->num_rows == 0) {
            ...
            if($data_user['status'] == 'suspend') {
                ...
                $response = $WATL->sendMessage(filter_phone('62',$postPhone),$Message);
                file_put_contents('.notifier', json_encode($response, JSON_PRETTY_PRINT));
                if($response == true) {
                    $_SESSION['login'] = ['otp' => $OTP, 'phone' => $postPhone, 'name' => $data_user['name'], 'email' => $data_user['email']];
    ...
    if($result_csrf == false) {
        ...
        if(check_bcrypt($postPin, $data_user['pin']) == true) {
            ...
            $WATL->sendMessage($data_user['phone'],$Message);
...
/*} else if(isset($_POST['reg'])) {
    ...
    if($result_csrf == false) {
        ...
        $result_mail = mailer($_MAILER, [
            'dest' => $post_email,
            'name' => $post_name,
            'subject' => 'OTP Untuk '.$_CONFIG['title'].'',
            'message' => base64_encode(mailplate('otp',[
                'name' => $post_name,
                'otp' => $OTP
            ])),
        ...
        file_put_contents('.notifier', json_encode($result_wa, JSON_PRETTY_PRINT));
        if($result_mail == true || $result_wa == false) {
    ...
    if($result_csrf == false) {
        ...
        $in = $call->query("INSERT INTO users VALUES ('','$card','$post_name','$post_email','$post_phone','$username','0','0','0','Basic','".strtoupper(random(6))."','$post_invited','active','default.png','No','$datetime', '".bcrypt($post_pin,10)."')");
        if($in == true){
            $call->query("INSERT INTO users_api VALUES ('".$username."','".random(8)."','".random(64)."','','development')");
            $kepo = $call->query("SELECT * FROM users WHERE username = '".$username."'")->fetch_assoc();
            setcookie('ssid', 'SHENN-A'.$kepo['id'].'AIY', $cookie_time, '/', $_SERVER['HTTP_HOST']);
            setcookie('token', $ShennCookie, $cookie_time, '/', $_SERVER['HTTP_HOST']);
            $call->query("INSERT INTO users_cookie VALUES ('$ShennCookie', '".$username."', '$datetime')");
            $_SESSION['user'] = $kepo;
            ...
            mailer($_MAILER, [
                ...
                'message' => base64_encode(mailplate('detail', ['user' => [
                    'user' => $username,
                    'email' => $post_email,
                    'phone' => $post_phone,
                    'name' => $post_name,
            ...
            $WATL->sendMessage($post_phone,"Hallo ".$post_name." \n\nSelamat datang di WhatsApp *{$_CONFIG['title']}* \n\nMelalui WhatsApp ini, Anda akan menerima informasi berupa notifikasi terkait Akun dan transaksi Anda di *{$_CONFIG['title']}* \n\n*MOHON JANGAN DI BLOKIR KONTAK INI*");
...
} else if(isset($_POST['resend_ot'])) {
    if($result_csrf == false) {
        $_SESSION['result'] = ['type' => false,'message' => 'System Error, please try again later.'];
    } else {
            
        $result_wa = $WATL->sendMessage(filter_phone('62',$_SESSION['login']['phone']),"Hallo ".$_SESSION['login']['name'].", \n\nJangan Berikan Kode Ini Kesiapapun Termasuk Pihak *{$_CONFIG['title']}*\n\nMasuk ke Akun dengan *Kode Verifikasi (OTP) : ".$_SESSION['login']['otp']."*\n\nMohon tidak memblokir nomor ini, hubungi customer service untuk bantuan.");
        $result_mail = mailer($_MAILER,[
            'dest' => $_SESSION['login']['email'],
            'name' => $_SESSION['login']['name'],
            'subject' => 'OTP Untuk '.$_CONFIG['title'].'',
            'message' => base64_encode(mailplate('otp',[
                'name' => $data['name'],
                'otp' => $_SESSION['login']['otp']
            ])),
            'is_template' => 'yes'
        ]);
